...
float total_voltage, current;
float state_of_charge_sensor, capacity_remaining_sensor;
int errors_bitmask_sensor_;
int delta_cell;
int power;

#define BATTERY_LOW_VOLTAGE 51.2
...
void setup() {
  ...
  lcd.setCursor(20, 8); // Set cursor position
...
void loop() {
  ...
  if (doConnect == true) {
    if (connectToServer()) {
      ...
      lcd.setFont(FONT_12x16);
      lcd.println(" Connected   "); // Display the bms connection
    } else {
...
void display() {
  ...
  if (connected == false) {
    ...
    else if (total_voltage <= 50) {
      lcd.setTextColor(TFT_RED, TFT_BLACK);
    }
    else {
      lcd.setTextColor(TFT_YELLOW, TFT_BLACK);
    }
  lcd.println(String(total_voltage) + "V  "); // Display the voltage
  ...
  if (connected == false) {
    ...
    else if (state_of_charge_sensor >= 66) {
      lcd.setTextColor(TFT_GREEN, TFT_BLACK);
    }
    else if (state_of_charge_sensor <= 33) {
      lcd.setTextColor(TFT_RED, TFT_BLACK);
    }
    else {
      lcd.setTextColor(TFT_YELLOW, TFT_BLACK);
    }
  lcd.println(String(state_of_charge_sensor) + "%  "); // Display the percentage
  lcd.setCursor(170, 90); // Set cursor position
  lcd.println(String(capacity_remaining_sensor) + "Ah  "); // Display the Ah
  ...
  lcd.println(String(current) + "A   "); // Display the amps
  lcd.setCursor(15, 185); // Set cursor position
  lcd.setFont(FONT_12x16);
  lcd.println(String(power) + "W   "); // Display the watt
  ...
  if (connected == false) {
    ...
    else if (average_cell_voltage_sensor <= 3.1) {
      lcd.setTextColor(TFT_RED, TFT_BLACK);
    }
    else if (average_cell_voltage_sensor <= 3.23) {
      lcd.setTextColor(TFT_YELLOW, TFT_BLACK);
    }
    else {
      lcd.setTextColor(TFT_GREEN, TFT_BLACK);
    }
  lcd.println("Ave:" + String(average_cell_voltage_sensor) + "V "); // Display the cell ave
  
  lcd.setCursor(170, 185); // Set cursor position
  if (delta_cell_voltage_sensor >= 0.07) {
      lcd.setTextColor(TFT_RED, TFT_BLACK);
    }
  ...
  lcd.setFont(FONT_12x16);
  delta_cell = int(delta_cell_voltage_sensor * 1000);
  lcd.println("Del: " + String(delta_cell) + "mV   "); // Display the cell delta

  lcd.setCursor(170, 215); // Set cursor position
  //lcd.setFont(FONT_12x16);
  if (errors_bitmask_sensor_ > 0) {
      lcd.setTextColor(TFT_RED, TFT_BLACK);
